...
Salesforce is rarely an island. In mature enterprise deployments like AESF, it sits at the center of a web of integrations: inbound data from Epic EHR sync jobs, outbound callouts to FastAPI middleware, bidirectional change events consumed by downstream ETL pipelines, and external systems pushing webhooks back in. Understanding the full menu of Salesforce integration patterns — and knowing when to reach for each one — is one of the clearest markers separating senior from staff-level engineers.

The AESF architecture embodies this complexity directly. An agent edits an `Account` in Salesforce, an Apex trigger fires, a `@future` callout hits the FastAPI middleware, the middleware enqueues a sync job, and Epic EHR is updated within seconds. That single user gesture crosses three systems, two authentication boundaries, and at least two async execution contexts. Every piece of that chain has a Salesforce integration pattern behind it — REST callouts, named credentials, platform events for observability, and outbound messages for simpler one-way flows.

This week covers the building blocks: REST and SOAP APIs (authentication, limits, error handling), outbound messages vs. Apex callouts, Platform Events vs. Change Data Capture vs. Streaming API, inbound webhooks, and connected apps. You will also see how Canvas fits into the picture for embedded UIs. Throughout, the AESF trigger→callout→middleware→Epic pattern is used as the concrete anchor — every concept maps back to something you have already shipped.

By the end of this week you should be able to design a new integration surface on AESF — decide between a platform event and a callout, choose the right auth flow, implement limit-safe error handling, and explain the trade-offs to a product team in plain language. That is the staff-level move: turning pattern knowledge into architectural judgment.

---

## 1. REST and SOAP APIs: Authentication

Salesforce exposes two major API families. The REST API (JSON/HTTP) is the modern default and is what AESF's FastAPI middleware uses. The SOAP API (XML/WSDL) is older but still prevalent in legacy insurance integrations — you may encounter it in Epic-adjacent systems. Both require OAuth 2.0 tokens on every request.

### OAuth 2.0 Flows in Salesforce

| Flow | Use Case | AESF Relevance |
|---|---|---|
...
req.setEndpoint('callout:AESF_Middleware/api/v2/accounts');
req.setMethod('POST');
...
| Limit Type | Scope | Default (Enterprise) | AESF Impact |
|---|---|---|---|
...
The `@future` limit is especially relevant to AESF. When an agent does a bulk import of 200 Accounts, the trigger fires 200 times in a single transaction. If each trigger invocation calls `@future`, only the first 50 enqueue — the rest are silently dropped unless you bulkify with a single `@future` call that accepts a `List<Id>`.

```apex
// AESF pattern: bulkified @future callout — one HTTP call per batch, not per record
public class AccountSyncHandler {
    @future(callout=true)
    public static void syncToMiddleware(List<Id> accountIds) {
        List<Account> accounts = [
            SELECT Id, Name, AESF__Epic_Client_Id__c, BillingStreet
            FROM Account
        ...
        HttpRequest req = new HttpRequest();
        req.setEndpoint('callout:AESF_Middleware/api/v2/accounts/bulk');
        req.setMethod('POST');
        req.setHeader('Content-Type', 'application/json');
        req.setBody(payload);
        ...
        if (res.getStatusCode() >= 400) {
            ...
            AESF__Sync_Error__e evt = new AESF__Sync_Error__e(
                AESF__Record_Ids__c = String.join(new List<Id>(accountIds), ','),
                AESF__Error_Message__c = res.getBody(),
                AESF__HTTP_Status__c = res.getStatusCode()
            );
...
**Common mistake:** Swallowing non-200 responses silently. Always publish a Platform Event or write to a custom error log object when a callout fails. Silent failures cause data divergence between Salesforce and Epic that takes hours to reconcile.
...
| AESF usage | Not used | Primary pattern |

AESF uses Apex callouts exclusively because the Epic middleware API is REST/JSON and the integration logic requires field mapping, conditional routing (insert vs. update vs. delete), and error publication — none of which outbound messages can handle.
...
Use case: custom application events — sync errors, workflow milestones, audit signals. In AESF, publishing `AESF__Sync_Error__e` events lets the middleware subscribe and trigger alerting without polling.
...
Use case: replicate Salesforce record changes to an external store (e.g., BigQuery). The `etl-appliedcrm-bq` pipeline could subscribe to CDC events for `AESF__Policy__c` instead of polling the REST API on a schedule.
...
```python
# FastAPI middleware subscribing to AESF Platform Events via Salesforce Streaming API
...
async def listen_for_sync_errors(access_token: str, instance_url: str):
    async with SalesforceStreamingClient(
        ...
        await client.subscribe("/event/AESF__Sync_Error__e")
        async for message in client:
            payload = message["data"]["payload"]
            record_ids = payload["AESF__Record_Ids__c"]
            error_msg = payload["AESF__Error_Message__c"]
...
```apex
@RestResource(urlMapping='/webhook/epic-sync-result/*')
global class EpicSyncResultWebhook {
    @HttpPost
    global static void handleResult() {
        ...
        String epicClientId = (String) body.get('epic_client_id');
        String status = (String) body.get('status');
        String errorMsg = (String) body.get('error_message');

        List<Account> accounts = [
            SELECT Id, AESF__Sync_Status__c, AESF__Last_Epic_Error__c
            FROM Account
            WHERE AESF__Epic_Client_Id__c = :epicClientId
            LIMIT 1
        ];

        if (!accounts.isEmpty()) {
            accounts[0].AESF__Sync_Status__c = status;
            accounts[0].AESF__Last_Epic_Error__c = errorMsg;
            update accounts;
        }
...
async def post_sync_result_to_salesforce(
    ...
    async with httpx.AsyncClient() as client:
        resp = await client.post(
            f"{instance_url}/services/apexrest/webhook/epic-sync-result/",
            headers={"Authorization": f"Bearer {sf_token}", "Content-Type": "application/json"},
            json={"epic_client_id": epic_client_id, "status": status, "error_message": error_message},
            timeout=30.0,
        )
        resp.raise_for_status()
```
...
## 6. The Salesforce → Middleware → Epic Pattern in AESF
...
FastAPI Middleware (/api/v2/accounts/bulk)
    │  deserialize, validate, map to Epic schema
    ▼
PostgreSQL queue table (aesf_sync_queue)
    │  worker polls, retries on failure
    ▼
Epic EHR REST API (/clients POST/PUT)
    │
    ▼
Middleware POSTs result back to Salesforce Apex REST endpoint
    │
    ▼
Account.AESF__Sync_Status__c updated
```
...
| Epic EHR | 4xx client error | Do not retry; alert and log; fix mapping |
| Epic EHR | 5xx / timeout | Exponential back-off in worker |
...
- **Certificate-based auth:** For JWT Bearer, upload the public key certificate to the connected app. The private key stays in the middleware's secret manager (Vault in AESF's case).
...
For AESF's bulk Account sync, `@future` works well for moderate volumes. If the integration needs to chain calls (e.g., create client in Epic, then fetch the assigned Epic ID, then update Salesforce), `Queueable` with chaining is the right pattern.

```apex
// Queueable callout with chaining: create Epic client, then write back Epic ID
public class EpicClientCreateJob implements Queueable, Database.AllowsCallouts {
    private List<Id> accountIds;

    public EpicClientCreateJob(List<Id> accountIds) {
    ...
    public void execute(QueueableContext ctx) {
        ...
        if (res.getStatusCode() == 202) {
            ...
            System.enqueueJob(new EpicClientPollJob(accountIds, jobId));
        }
    }

    private String buildPayload(List<Id> ids) {
        List<Account> accs = [SELECT Id, Name, AESF__Epic_Client_Id__c FROM Account WHERE Id IN :ids];
...
Do not pass raw Salesforce JSON to Epic. The middleware's job is to validate, transform, and enrich. Use Pydantic models to enforce the Epic API's contract and surface mapping errors early.

### Circuit Breaker

If Epic EHR returns 5xx errors for 10 consecutive calls, stop sending for 60 seconds and alert. This prevents the queue from growing unboundedly while Epic is down and avoids hammering a recovering service.
...
└── AESF Architecture
    ...
    ├── Middleware → PostgreSQL queue → Epic EHR
    ├── Epic result → Apex REST webhook → Account update
...
**2.** AESF's `AccountTriggerHandler` calls a `@future(callout=true)` method. A batch DML operation inserts 200 Accounts in one transaction. How many `@future` invocations actually execute, and why?

**3.** What is the key operational difference between Outbound Messages and Apex callouts regarding delivery guarantees?

**4.** A Platform Event subscriber receives a `AESF__Sync_Error__e` event. The `replayId` is 4500. What does this number enable?

**5.** Change Data Capture fires when an `AESF__Policy__c` record is updated. The event arrives at the middleware subscriber. What fields are guaranteed to be present in the payload, and what is NOT included?
...
**8.** What is the difference between `@future(callout=true)` and a Queueable implementing `Database.AllowsCallouts`, and when would you choose Queueable in AESF?

**9.** An Apex REST endpoint at `/services/apexrest/webhook/epic-sync-result/` receives POST requests from the FastAPI middleware. What authentication mechanism should the middleware use, and why not anonymous?

**10.** What does `hmac.compare_digest()` protect against compared to the `==` operator when validating a Canvas `signed_request`?

**11.** A new engineer proposes using a PushTopic to stream `AESF__Policy__c` changes to the BigQuery ETL pipeline. What is the preferred modern alternative and why?

**12.** The AESF middleware's `/api/v2/accounts/bulk` endpoint receives the same `sf_record_id` twice in quick succession (Salesforce retry). What design property prevents a duplicate Epic API call?
...
**15.** The AESF circuit breaker stops sending requests to Epic after 10 consecutive 5xx errors. From the Salesforce side, what happens to the sync queue during the 60-second pause?

**16.** What OAuth scope should the AESF middleware's Connected App have, and why not `full`?
...
**19.** In the AESF sync architecture, why is PostgreSQL used as a queue between the middleware and Epic, rather than calling Epic directly in the FastAPI request handler?
...
??? note "Reveal Answers"
    ...
    **8.** `@future` is simpler but cannot chain (you cannot enqueue another job from inside it) and accepts only primitive or primitive-collection parameters. Queueable with `Database.AllowsCallouts` supports chaining — a Queueable can call `System.enqueueJob()` to spawn a follow-up job — and accepts complex object parameters. In AESF, Queueable is preferred when the sync requires two sequential HTTP calls: for example, POST to Epic to create the client (getting an assigned Epic ID back), then write that Epic ID back to Salesforce. A single `@future` cannot do this because it cannot chain.
    ...
    **12.** Idempotency — the middleware uses the `sf_record_id` as the natural key in the `SyncQueue` table. On receiving a duplicate payload, the middleware finds the existing queue row and updates it (payload and status) rather than inserting a new row. The Epic worker processes one queue row per record, so the duplicate callout results in one Epic API call, not two. This is critical because Salesforce `@future` methods can be invoked multiple times in error-retry scenarios.
