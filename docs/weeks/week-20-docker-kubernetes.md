...
The week closes with Helm chart authoring grounded in the `infra-manifests` repository you already own. The integration platform runs three Kubernetes services — module-crm-middleware, module-etl-pipeline, and module-ehr-sdk — each with environment-specific `values.*.yaml` files. Writing and reading Helm charts with that concrete context makes the templating concepts stick and surfaces the gaps that cause staging-to-production drift.
...
**Common mistake:** Setting `requests` much lower than `limits` (wide QoS class `Burstable`) on crm-middleware pods. Under memory pressure the kubelet will evict Burstable pods before Guaranteed pods. The sync worker, which processes EHR webhook callbacks, should have `requests == limits` to be placed in the Guaranteed QoS class and avoid unexpected eviction during peak load.
...
### CRM Middleware Multi-Stage Dockerfile
...
LABEL org.opencontainers.image.revision="${GIT_SHA}" \
      org.opencontainers.image.source="github.com/your-org/crm-middleware"
...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
        - name: middleware
          image: gcr.io/your-project/crm-middleware:{{ .Values.image.tag }}
          ports:
            - containerPort: 8000
          resources:
            requests:
...
`maxUnavailable: 0` with `maxSurge: 1` is the zero-downtime pattern for crm-middleware — critical because EHR callback webhooks arrive continuously and a brief gap in the receiver causes missed sync events that require manual reconciliation.
...
In the integration platform ecosystem, PostgreSQL (if self-hosted) or a Redis queue would use a StatefulSet. The key difference from a Deployment: pods are created and deleted in order, and `kubectl rollout restart` on a StatefulSet terminates `pod-N` and waits for it to become Ready before proceeding to `pod-(N-1)`.

### DaemonSet

A DaemonSet ensures exactly one pod runs on every node (or on nodes matching a selector). Common uses: log shippers (Fluentd), node-level metrics exporters (Datadog agent), and CNI plugins. In GKE the Datadog DaemonSet that monitors node-level metrics is managed this way.
...
HPA scales a Deployment (or StatefulSet) based on observed metrics versus a target. The most common metric is CPU utilization, but the integration platform's sync worker is a better candidate for a custom metric like queue depth.
...
For the ETL sync worker, scaling on CPU is a lagging indicator — the queue fills up before CPU climbs. A better approach is to expose a `/metrics` endpoint from the worker with a Prometheus gauge for pending job count, and configure the HPA to target that metric via the Prometheus Adapter.
...
**Common mistake:** Setting `minReplicas: 1` for a stateless HTTP service in production. A single pod means any rollout, node drain, or OOM event causes a brief outage. For crm-middleware, `minReplicas: 2` across two availability zones is the correct baseline.
...
```yaml
apiVersion: v2
name: module-crm-middleware
description: CRM-EHR Integration Platform Python FastAPI middleware
type: application
...
image:
  repository: gcr.io/your-project/crm-middleware
  tag: latest
...
ingress:
  enabled: true
  host: api-staging.crm.yourdomain.com
  tlsSecret: crm-staging-tls
...
env:
  LOG_LEVEL: "debug"
  EHR_SERVER: "ehr-server-prod"
```

### _helpers.tpl

```
{{- define "module-crm-middleware.fullname" -}}
{{- printf "%s-%s" .Release.Name .Chart.Name | trunc 63 | trimSuffix "-" }}
{{- end }}

{{- define "module-crm-middleware.labels" -}}
...
kind: Deployment
metadata:
  name: {{ include "module-crm-middleware.fullname" . }}
  labels:
    {{ include "module-crm-middleware.labels" . | nindent 4 }}
spec:
...
```bash
# Deploy to staging
helm upgrade --install crm-middleware ./charts/module-crm-middleware \
  -f charts/module-crm-middleware/values.yaml \
  -f charts/module-crm-middleware/values.staging.yaml \
  --namespace crm-staging \
  --set image.tag=$(git rev-parse --short HEAD)

# Dry-run to preview rendered manifests
helm template crm-middleware ./charts/module-crm-middleware \
  -f charts/module-crm-middleware/values.staging.yaml | kubectl diff -f -
```
...
**7.** A `NetworkPolicy` is applied to the `crm-middleware` pods with `policyTypes: [Ingress, Egress]` but the egress rules only allow port 5432 to the database. What happens to outbound DNS queries from those pods?
...
**14.** You run `kubectl delete pod crm-middleware-abc123` in the production namespace. A new pod appears 5 seconds later. Why?
...
**16.** In the integration platform's Helm chart, `values.staging.yaml` sets `autoscaling.enabled: true` but `values.yaml` sets it to `false`. The deployment template has `{{- if not .Values.autoscaling.enabled }} replicas: {{ .Values.replicaCount }} {{- end }}`. What happens to `spec.replicas` in the staging deployment?
...
**19.** Why should secret values never appear in `values.*.yaml` files committed to a git repository, and what is the correct alternative for the integration platform?
...
??? note "Reveal Answers"
    ...
    **8.** The stabilization window prevents flapping by requiring the desired replica count to be stable for the specified number of seconds before acting. A higher scale-down window (e.g., 300 seconds) prevents the HPA from prematurely removing pods during a brief lull in traffic, which would then require immediately scaling back up. Scale-up uses a shorter window because the cost of being under-provisioned (dropped requests, latency spikes) is higher than briefly over-provisioning. For the integration platform's sync worker, a 5-minute scale-down window prevents repeatedly cycling pods during bursty EHR sync events.
    ...
    **15.** A pod where requests equal limits for all resources is assigned the `Guaranteed` QoS class. This is the highest priority class; the kubelet will only evict Guaranteed pods as a last resort, after all `BestEffort` and `Burstable` pods have been evicted. For sync workers that hold in-memory state about EHR webhook callbacks being processed, unexpected eviction could cause duplicate processing or data loss. Setting requests equal to limits on those pods prevents eviction under node memory pressure.
