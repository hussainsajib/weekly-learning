...
Every change you make in Setup UI or via tooling is a metadata change. When you create a custom field `APP__Policy_Number__c` on `APP__Policy__c`, Salesforce generates an XML metadata file internally that describes that field. SFDX exposes this as a real file you can version-control:

```xml
<!-- force-app/main/default/objects/APP__Policy__c/fields/APP__Policy_Number__c.field-meta.xml -->
<?xml version="1.0" encoding="UTF-8"?>
<CustomField xmlns="http://soap.sforce.com/2006/04/metadata">
    <fullName>APP__Policy_Number__c</fullName>
...
**Common mistake:** Treating a Lookup like a foreign key with guaranteed referential integrity. If the parent record is deleted and the lookup is optional, child records become orphaned with a null value — not an error. In the integration platform, `APP__Line__c` has a required Master-Detail to `APP__Policy__c`, which is intentional: deleting a policy cascades to lines, mirroring the EHR system behavior.
...
sf org login web --alias crm-qa --instance-url https://test.salesforce.com
...
sf project deploy start --source-dir force-app --target-org crm-qa

# Run Apex tests
sf apex run test --class-names AccountTriggerHandlerTest --target-org crm-qa --result-format human

# Execute anonymous Apex
sf apex run --file scripts/apex/seed-data.apex --target-org crm-qa

# Query SOQL
sf data query --query "SELECT Id, Name FROM APP__Policy__c LIMIT 10" --target-org crm-qa
```

**SFDX project structure** for the integration platform package looks like:
...
│           └── objects/
│               └── APP__Policy__c/
│                   ├── APP__Policy__c.object-meta.xml
│                   └── fields/
```
...
```bash
# Create package
sf package create --name "CRM Core" --package-type Unlocked --path force-app --target-dev-hub devhub

# Create a new version
sf package version create --package "CRM Core" --installation-key-bypass --wait 20

# Install a specific version in an org
sf package install --package 04t... --target-org crm-staging --wait 10
```

Unlocked packages support dependencies between packages, making them suitable for the integration platform architecture where `sfdc-crm-package` could declare a dependency on `sfdc-ams-package`.

### SFDX Source Deploy (current integration platform workflow)

For orgs where you are not yet using packages, `sf project deploy start` deploys the full source directory or a subset. This is what the integration platform CI/CD pipeline currently uses:

```bash
# Deploy with test run, fail fast
sf project deploy start \
  --source-dir force-app \
  --target-org crm-staging \
...
**Permission Set Groups** (introduced in API v51) bundle multiple permission sets into a single assignable unit — useful for the integration platform where the "Integration User" needs a specific combination of object permissions.
...
In the integration platform, the Integration User that calls the crm-middleware API typically runs with a System Administrator profile (or a custom profile with equivalent object access) because it needs to read and write all integration platform objects. This is a deliberate security trade-off documented in the architecture.
...
### Integration Platform Trigger Design and Governor Limits

In the integration platform, `AccountTriggerHandler.syncToEHR` makes an HTTP callout to the crm-middleware API. HTTP callouts are prohibited in synchronous triggers — they must be deferred to `@future` methods or Queueable Apex:

```apex
// Correct pattern used in the integration platform — defer callout to @future
public class AccountTriggerHandler {
    public static void syncToEHR(List<Account> newAccounts, Map<Id, Account> oldMap) {
        ...
        if (!accountIdsToSync.isEmpty()) {
            EHRSyncFuture.syncAccounts(new List<Id>(accountIdsToSync));
        }
    }
}

public class EHRSyncFuture {
    @future(callout=true)
    public static void syncAccounts(List<Id> accountIds) {
        ...
        EHRMiddlewareClient.syncAccounts(accounts);
    }
}
```
...
In the integration platform ecosystem:
- `APP` — the CRM-EHR Integration Platform package namespace
- the AMS package — a separate Agency Management System package with its own namespace

Every API name in a managed package is prefixed: `APP__Policy__c`, `APP__Line__c`, `AMS__Agency__c`. This prevents naming conflicts when two packages are installed in the same org.
...
```apex
// Inside the integration platform package (no prefix needed for own components)
APP__Policy__c policy = new APP__Policy__c();

// In subscriber org Apex calling into the installed integration platform package
APP__Policy__c policy = new APP__Policy__c(
    APP__Policy_Number__c = 'POL-001',
    APP__EHR_Policy_Id__c = '12345'
);
```
...
```sql
SELECT Id, APP__Policy_Number__c, APP__EHR_Policy_Id__c,
       APP__Account__r.Name
FROM APP__Policy__c
WHERE APP__EHR_Policy_Id__c != null
LIMIT 200
```
...
This is directly relevant to integration platform maintenance: adding a new field to `APP__Policy__c` is safe and fully backward-compatible. Renaming it or changing its type requires a new field plus a migration — the old field cannot be removed while subscribers exist.
...
SELECT Id, Name,
    (SELECT Id, APP__Line_Number__c, APP__Premium__c
     FROM APP__Lines__r)
FROM APP__Policy__c
WHERE APP__Account__c = :accountId

-- Child-to-parent (dot notation)
SELECT Id, APP__Policy_Number__c,
       APP__Account__r.Name,
       APP__Account__r.BillingCity
FROM APP__Policy__c
WHERE APP__Status__c = 'Active'

-- Aggregate
SELECT APP__Account__c, COUNT(Id) policyCount, SUM(APP__Total_Premium__c) totalPremium
FROM APP__Policy__c
WHERE APP__Status__c = 'Active'
GROUP BY APP__Account__c
HAVING COUNT(Id) > 5
```
...
List<APP__Policy__c> policies = [
    SELECT Id, APP__Policy_Number__c
    FROM APP__Policy__c
    WHERE APP__Status__c = :status
    AND APP__Total_Premium__c >= :minPremium
...
List<List<SObject>> results = [
    ...
    RETURNING Account(Id, Name), Contact(Id, Name, Email),
              APP__Policy__c(Id, APP__Policy_Number__c)
    LIMIT 20
];
```
...
private class AccountTriggerHandlerTest {
    ...
    @IsTest
    static void testSyncToEHR_UpdateTriggersCallout() {
        Account acc = [SELECT Id FROM Account LIMIT 1];

        // Mock the HTTP callout
        Test.setMock(HttpCalloutMock.class, new EHRMiddlewareMock());
        ...
        Account updated = [SELECT APP__Sync_Status__c FROM Account WHERE Id = :acc.Id];
        System.assertEquals('Synced', updated.APP__Sync_Status__c,
            'Expected sync status to be updated after EHR callout');
    }
}
```
...
The integration platform architecture relies heavily on async Apex because synchronous triggers cannot make HTTP callouts. The four async options and when to use each:
...
**Queueable example** — useful for the integration platform's EHR sync because it provides a job ID for monitoring:

```apex
public class EHRAccountSyncJob implements Queueable, Database.AllowsCallouts {
    private List<Id> accountIds;

    public EHRAccountSyncJob(List<Id> accountIds) {
...
// In trigger handler:
System.enqueueJob(new EHRAccountSyncJob(new List<Id>(accountIdsToSync)));
```

**Batch Apex** for the nightly reconciliation job that compares integration platform records against the EHR system:

```apex
public class EHRReconciliationBatch implements Database.Batchable<SObject>, Database.AllowsCallouts {
    public Database.QueryLocator start(Database.BatchableContext bc) {
        return Database.getQueryLocator([
            SELECT Id, APP__EHR_Client_Id__c, APP__Last_Sync__c
            FROM Account
            WHERE APP__EHR_Client_Id__c != null
        ]);
    }

    public void execute(Database.BatchableContext bc, List<Account> scope) {
        // Process each chunk of 200 (default) accounts
        EHRMiddlewareClient.reconcileAccounts(scope);
    }
...
Database.executeBatch(new EHRReconciliationBatch(), 50); // 50 records per chunk
```
...
│       └── Custom Objects (APP__Policy__c, APP__Line__c...)
│
├── Tooling
...
│
├── Namespaces & Packages
│   ├── APP namespace → APP__ prefix
│   ├── AMS package namespace → AMS__ prefix
...
**2.** You have an `after insert` trigger on `APP__Policy__c` that needs to call the crm-middleware API. Why can't you make the HTTP callout directly inside the trigger, and what is the correct pattern?
...
**6.** In the integration platform security model, the Integration User runs with broad object-level access. What Apex keyword would you use in a trigger handler to ensure it runs in system context and bypasses sharing rules, and what keyword would you use in an Aura controller to respect the current user's record-level access?

**7.** You need to query `APP__Policy__c` records and include related `APP__Line__c` records for each policy. Write a SOQL query that retrieves policy number, total premium, and the line number and premium for each related line.

**8.** What is the maximum number of `@future` method calls allowed in a single transaction, and what pattern do you use to stay within this limit when an update to 200 Account records all need to sync to the EHR system?

**9.** Describe the difference between Unlocked Packages and Managed Packages. Which one is the integration platform namespace package type, and what constraint does that impose on removing public API components?

**10.** What does `WITH SECURITY_ENFORCED` do in a SOQL query, and when should you use it in the integration platform context vs. when should you avoid it?
...
**14.** In a managed package, you realize that a custom field `APP__Old_Status__c` (Text type) needs to be a Picklist. You have active subscribers. What is the correct migration path?

**15.** What is SOSL and when would you use it instead of SOQL? Give a use case relevant to the integration platform.
...
**18.** Why does the integration platform integration user need `without sharing` semantics in its data access classes, even if those classes are called from within a trigger?

**19.** In the SFDX project for `sfdc-crm-package`, what file type accompanies every Apex class file, and what does it contain?

**20.** You run `sf apex run test --class-names AccountTriggerHandlerTest --target-org crm-staging` and get 98% coverage on `AccountTriggerHandler.cls` but the production deployment still fails the coverage check. Why might this happen?
...
??? note "Reveal Answers"
    ...
    **7.**
    ```sql
    SELECT Id, APP__Policy_Number__c, APP__Total_Premium__c,
           (SELECT Id, APP__Line_Number__c, APP__Premium__c
            FROM APP__Lines__r)
    FROM APP__Policy__c
    WHERE APP__Status__c = 'Active'
    LIMIT 200
    ```
    The child relationship name `APP__Lines__r` is the plural form of the child object's relationship name as defined on the Master-Detail field, suffixed with `__r` for custom relationships.
    ...
    **14.** You cannot change a field's type in a released managed package. The migration path is: (1) create a new field `APP__Status__c` of type Picklist with the desired values; (2) write a one-time data migration script (anonymous Apex or batch) that copies values from `APP__Old_Status__c` to `APP__Status__c`; (3) update all Apex code, Flows, reports, and page layouts in the next package version to reference the new field; (4) mark `APP__Old_Status__c` as `@deprecated` in any Apex references; (5) communicate the deprecation in release notes. The old field cannot be deleted while any subscriber org is still on a package version that references it.

    **15.** SOSL (Salesforce Object Search Language) performs full-text search across multiple object types in a single operation, using Salesforce's search index rather than the database query engine. Use it when the user provides a free-text search term and you don't know which object type holds the matching record. In the integration platform, a support lookup feature might let a user search for "John Smith" and find matching Accounts, Contacts, and `APP__Policy__c` records simultaneously — a single SOSL query handles this more efficiently than three separate SOQL LIKE queries.
